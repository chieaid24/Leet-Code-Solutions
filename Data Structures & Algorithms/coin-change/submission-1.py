class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        # we can store the value in the dp array, instead of doing currMin, since we already
        # allocated memory no need to create a new variable!
        dp = [float("inf")] * (amount + 1)
        dp[0] = 0
        for amt in range(1, len(dp)):
            # loop through each coin and subtract it
            for coin in coins:
                if amt - coin >= 0 and dp[amt - coin] != float("inf"): # don't need this bc if we are looking for a min, and invalid, it will be comparing inf + 1 to inf, so inf will remain the value, so we can just have it execute
